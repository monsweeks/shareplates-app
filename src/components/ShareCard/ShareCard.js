...
class ShareCard extends React.PureComponent {
  render() {
    ...
    const joinedAdminUser = share.shareUsers.find((shareUser) => shareUser.shareRoleCode === 'ADMIN' && shareUser.status === 'ONLINE');
      ...
      <Card className={`share-card-wrapper g-no-select ${className}`}>
        <CardBody>
          ...
          <div className="tag-info">
            <span className={`short-icon admin-join ${joinedAdminUser ? 'joined' : ''}`}>
              <i className="fal fa-head-side" />
            </span>
            <span className={`short-icon status  ${share.startedYn ? 'started' : ''}`}>
              {share.startedYn && <i className="fal fa-play" />}
              {!share.startedYn && <i className="fal fa-pause" />}
            </span>
            <span className={`short-icon user-count-icon ${totalUserCount > 0 ? 'has-joined-user' : ''}`}>
